...
function money(cents, currency = "NGN") {
  return formatMoney(cents, currency, "en");
}

function returnPage(payment, plan, { status = "pending" }) {
  const title =
    status === "paid" ? "Payment received" : status === "failed" ? "Payment did not go through" : "Waiting for Paystack";
  const msg =
    status === "paid"
      ? "You're in. Access is open — go back to Telegram."
      : status === "failed"
        ? "The payment did not go through. You can try again from the bot."
        : "Paystack will confirm once you finish on your phone. You can close this page and return to Telegram.";
  return `<!DOCTYPE html>
<html lang="en"><head>
...
<body><div class="card">
  ...
  <div class="row"><span>Plan</span><b>${plan?.name || payment?.planCode || "—"}</b></div>
  <div class="row"><span>Amount</span><b>${payment ? money(payment.amountCents, payment.currency) : "—"}</b></div>
</div></body></html>`;
}

export function startHttpServer({ config, store, paystack, publicDir }) {
  ...
  async function handleApi(req, res, url) {
    ...
    if (path === "/health") {
      json(res, 200, {
        ...
        paystack: { configured: paystack.configured, env: paystack.env, api: paystack.apiUrl },
      });
      return true;
    }
    ...
    if (path === "/webhooks/paystack" && method === "POST") {
      if (!paystack.configured) {
        send(res, 503, "paystack not configured", { "Content-Type": "text/plain; charset=utf-8" });
        return true;
      }
      const raw = await readBody(req);
      if (!paystack.verifySignature(raw, req.headers)) {
        send(res, 403, "invalid signature", { "Content-Type": "text/plain; charset=utf-8" });
        return true;
      }
      let event;
      try {
        ...
        send(res, 400, "invalid body", { "Content-Type": "text/plain; charset=utf-8" });
        return true;
      }
      try {
        await paystack.fulfillFromEvent(event);
      } catch (e) {
        console.warn("paystack webhook:", e.message);
      }
    ...
    if (path === "/pay/return" && method === "GET") {
      ...
      if (payment) payment = (await paystack.fulfillByPid(payment.id)) || payment;
    ...
    if (path === "/api/admin/me" && method === "GET") {
      json(res, 200, { ok: true, user: auth.user, paystack: paystack.env, settings: store.settings() });
      return true;
    }
    ...
    if (path.startsWith("/api/admin/users/") && method === "GET") {
      ...
      if (!user) json(res, 404, { ok: false, error: "User not found" });
    ...
    if (path === "/api/admin/subscriptions" && method === "POST") {
      ...
      if (!sub) json(res, 400, { ok: false, error: "Invalid user or plan" });
    ...
    if (path.startsWith("/api/admin/subscriptions/") && method === "PATCH") {
      ...
      if (!sub) json(res, 404, { ok: false, error: "Subscription not found" });
    ...
    if (path.startsWith("/api/admin/payments/") && path.endsWith("/confirm") && method === "POST") {
      ...
      const payment = paystack.markPaid(id, { metadata: { manual: true } });
      if (!payment) json(res, 404, { ok: false, error: "Payment not found" });
    ...
    if (path === "/api/admin/settings" && method === "GET") {
      json(res, 200, {
        ...
        paystack: {
          ...
          env: paystack.env,
          apiUrl: paystack.apiUrl,
          currency: paystack.currency,
          webhook: `${config.publicUrl}/webhooks/paystack`,
