  ...
  return [
    {
      ...
      code: "weekly",
      name: "Hebdomadaire",
      description: "7 jours d'accès au bot.",
      priceCents: 5000,
      currency: "XOF",
      interval: "week",
      credits: 9999,
    ...
    },
    {
      ...
      code: "monthly",
      name: "Mensuel",
      description: "30 jours d'accès au bot.",
      priceCents: 18000,
      currency: "XOF",
      interval: "month",
      credits: 9999,
    ...
    },
    {
      ...
      code: "annual",
      name: "Annuel",
      description: "Un an d'accès au bot.",
      priceCents: 210000,
      currency: "XOF",
      interval: "year",
      credits: 9999,
...
function syncOfficialPlans(state) {
  ...
  state.settings = { ...state.settings, currency: "XOF" };
}
...
export function createStore(filePath, { adminEmail, adminPassword } = {}) {
  ...
  const api = {
    ...
    registerBotUser({ from, email, password, displayName, lang, referralCode }) {
      ...
      if (!telegramId) throw authError("NO_TELEGRAM", "Compte Telegram manquant.");
      if (name.length < 2) throw authError("INVALID_NAME", "Nom trop court.");
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailN)) throw authError("INVALID_EMAIL", "Email invalide.");
      if (pass.length < 6) throw authError("INVALID_PASSWORD", "Mot de passe trop court.");

      const byTg = state.users.find((x) => Number(x.telegramId) === telegramId && x.role !== "admin") || null;
      if (byTg?.email && byTg.passwordHash) {
        throw authError("ALREADY_REGISTERED", "Compte déjà créé.");
      }
      const byEmail = findUserByEmail(emailN);
      if (byEmail && byEmail.role === "admin") {
        throw authError("EMAIL_TAKEN", "Cet email est déjà pris.");
      }
    ...
    loginBotUser({ from, email, password }) {
      ...
      if (!u || u.role === "admin" || u.status === "banned" || !u.passwordHash) {
        throw authError("LOGIN_BAD", "Email ou mot de passe incorrect.");
      }
      if (!verifyPassword(password, u.passwordHash)) {
        throw authError("LOGIN_BAD", "Email ou mot de passe incorrect.");
      }
      if (u.telegramId && Number(u.telegramId) !== telegramId) {
        throw authError("TELEGRAM_MISMATCH", "Ce compte est déjà lié à un autre Telegram.");
      }
      u.telegramId = telegramId;
      u.username = from?.username || u.username;
      u.lastSeenAt = nowIso();
    ...
    activateSubscription({ userId, plan, paymentId = null, provider = "tchin", cancelAtPeriodEnd = true }) {
      const t = nowIso();
      ...
      return (
        state.payments.find(
          (p) =>
            p.providerRef === want ||
            p.id === want ||
            String(p.metadata?.tchinToken || "") === want,
        ) || null
      );
    },
